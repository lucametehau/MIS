...
\begin{itemize}
    \item \textbf{Active Node Vector}: Instead of iterating over all $N$ vertices in every round, we maintain a vector of \texttt{active\_nodes}.
    \item \textbf{Dynamic Scheduling}: For the neighbor-checking phase, we use \texttt{schedule(dynamic, 32)} to balance load on irregular graphs.
    \item \textbf{Efficient Filtering}: We use a parallel filter to construct the \texttt{active\_nodes} vector for subsequent rounds.
\end{itemize}
