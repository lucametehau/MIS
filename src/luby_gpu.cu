...
__global__ void luby_cooperative_kernel(
    const uint32_t* offsets, const Node* edges,
    uint8_t* is_active, uint8_t* in_mis,
    uint8_t* candidates, int* d_any_active, int n)
{
    namespace cg = cooperative_groups;
    cg::grid_group grid = cg::this_grid();

    // Initialization
    for (int u = grid.thread_rank(); u < n; u += grid.size()) {
        is_active[u] = 1;
        in_mis[u] = 0;
    }
    grid.sync();
    ...
    int iteration_num = 0;
    while (true) {
        if (grid.thread_rank() == 0) *d_any_active = 0;
        grid.sync();
...
NodeList luby_gpu_mis2(const GraphCSR& g) {
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, luby_cooperative_kernel, blockSize, 0);
    ...
    int gridSize = deviceProp.multiProcessorCount * numBlocksPerSm;
    if (n < gridSize * blockSize) {
        gridSize = (n + blockSize - 1) / blockSize;
    }

    void* args[] = { &d_offsets, &d_edges, &d_is_active, &d_in_mis, &d_candidates, &d_any_active, &n };
    cudaLaunchCooperativeKernel((void*)luby_cooperative_kernel, gridSize, blockSize, args);
    cudaDeviceSynchronize();
